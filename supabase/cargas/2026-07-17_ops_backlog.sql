...
-- Origen WhatsApp (OP_WA_####), etapa En Cola (no descuenta MP aun),
...
do $$
declare v_cli uuid; v_op uuid; v_wa smallint; v_cola smallint;
begin
  if exists (select 1 from ordenes_pedido where numero like 'OP_WA_%') then
    raise notice 'Las OPs OP_BFP ya estan cargadas; no se repiten.'; return;
  end if;
  select id into v_wa from origenes_op where clave='whatsapp';
  select id into v_cola from etapas_produccion where nombre='En Cola';

  -- OP_WA_0001 · David Freyre
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0001',v_cli,(select id from ciudades where nombre='Medellín' and departamento='Antioquia'),'B2C',v_wa,v_cola,true,'2026-07-27','Incluye ademas: piso de caucho 9m2, agarres (tobillo/espalda/lazo), mueble mixto 0.9x1.5x0.45m, almacenador de fitball. CON INSTALACION Y ENVIO. Vendedor Yohan S.') returning id into v_op;
  ...
  -- OP_WA_0002 · Diana Marcela Gomez Rivera
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0002',v_cli,(select id from ciudades where nombre='Medellín' and departamento='Antioquia'),'B2C',v_wa,v_cola,true,'2026-07-25','Incluye ademas: agarres (lazo/tobillo/individual/espalda), rollos rack pad, almacenador de discos 3N, kit bandas de poder. CON INSTALACION Y ENVIO. Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
  ...
  -- OP_WA_0003 · Diego Alejandro Sanchez Sanchez
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0003',v_cli,(select id from ciudades where nombre='Facatativá' and departamento='Cundinamarca'),'B2C',v_wa,v_cola,false,'2026-07-16','Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
    select v_op,p.id,x.cant,p.precio_lista from (values ('1RaPF5f',1),('3ADR',2)) x(sku,cant) join productos p on p.sku=x.sku;

  -- OP_WA_0004 · Diego Julian Ospina Castillo
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0004',v_cli,(select id from ciudades where nombre='Tuluá' and departamento='Valle del Cauca'),'B2C',v_wa,v_cola,false,'2026-07-31','Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
    select v_op,p.id,x.cant,p.precio_lista from (values ('6BaPle',1)) x(sku,cant) join productos p on p.sku=x.sku;

  -- OP_WA_0005 · Fidel Manuel Barrio Zarate
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0005',v_cli,(select id from ciudades where nombre='Envigado' and departamento='Antioquia'),'B2C',v_wa,v_cola,false,'2026-07-31','Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
    select v_op,p.id,x.cant,p.precio_lista from (values ('1RaPF5f',1)) x(sku,cant) join productos p on p.sku=x.sku;

  -- OP_WA_0006 · Habitare espacios SAS
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0006',v_cli,(select id from ciudades where nombre='Cali' and departamento='Valle del Cauca'),'B2B',v_wa,v_cola,true,'2026-07-13','Incluye ademas: discos encauchetados 5/10/15/25/35/45 lbs (~2 c/u). INCLUYE INSTALACION Y ENVIO. Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
  ...
  -- OP_WA_0007 · Hector Andres Acosta Perez
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0007',v_cli,(select id from ciudades where nombre='Bogotá' and departamento='Cundinamarca'),'B2C',v_wa,v_cola,false,'2026-07-12','Incluye ademas: discos 35lbs x2, discos 45lbs x2. Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
    select v_op,p.id,x.cant,p.precio_lista from (values ('1RaPF10V2',1)) x(sku,cant) join productos p on p.sku=x.sku;

  -- OP_WA_0008 · Juan Carlos Rodriguez Santos
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0008',v_cli,(select id from ciudades where nombre='Cali' and departamento='Valle del Cauca'),'B2C',v_wa,v_cola,true,'2026-07-25','Incluye ademas: agarres (espalda/lazo/individual), barra olimpica cromada 15kg, discos 15/25/45 lbs, piso de caucho Sponch. Alm. agarres con 10 ganchos. CON INSTALACION Y ENVIO. Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
  ...
  -- OP_WA_0009 · Juan Pablo Valbuena Anaya
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0009',v_cli,(select id from ciudades where nombre='Cartagena' and departamento='Bolívar'),'B2C',v_wa,v_cola,false,'2026-07-16','Incluye ademas: barras olimpicas negras 15/20kg, agarres varios, mancuernas hex 5-45kg, discos 10/15/25/35 lbs. Parte de implementos ya DESPACHADOS (resaltados). Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
  ...
  -- OP_WA_0010 · Julian Fernando Mejia Nino
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0010',v_cli,(select id from ciudades where nombre='Sincelejo' and departamento='Sucre'),'B2C',v_wa,v_cola,false,'2026-07-25','DC Bar con gancho soldado arriba en la barra de pull up (para colgar TRX). Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
    select v_op,p.id,x.cant,p.precio_lista from (values ('7DcBar',1)) x(sku,cant) join productos p on p.sku=x.sku;

  -- OP_WA_0011 · Kevin Criollo
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0011',v_cli,(select id from ciudades where nombre='Medellín' and departamento='Antioquia'),'B2C',v_wa,v_cola,false,'2026-07-27','PF5 abatible tipo Andres Bernal (para tener bloques). Incluye ademas: barra olimpica negra 15kg, discos 10lbs x4 y 15lbs x4, agarres (individual/tobillo/espalda/lazo). Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
  ...
  -- OP_WA_0012 · Leonardo Fabio Duran Varela
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0012',v_cli,(select id from ciudades where nombre='Puebloviejo' and departamento='Magdalena'),'B2C',v_wa,v_cola,false,'2026-07-16','PF5 abatible. Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
    select v_op,p.id,x.cant,p.precio_lista from (values ('1RaPF5p',1)) x(sku,cant) join productos p on p.sku=x.sku;

  -- OP_WA_0013 · Maria Fernanda (Angolu)
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0014',v_cli,(select id from ciudades where nombre='Medellín' and departamento='Antioquia'),'B2C',v_wa,v_cola,false,'2026-07-13','Incluye ademas: barra crossfit semiprofesional 2m, discos 20kg x2, discos 10kg x2. Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
    select v_op,p.id,x.cant,p.precio_lista from (values ('1RaPF5f',1),('3PoMov',1)) x(sku,cant) join productos p on p.sku=x.sku;

  -- OP_WA_0015 · Sebastian Perez Morales
  insert into clientes (tipo,nombre,nit_cedula,telefono,ciudad_id,direccion)
  ...
  insert into ordenes_pedido (numero,cliente_id,ciudad_id,segmento,origen_id,etapa_id,requiere_instalacion,fecha_entrega_pactada,notas)
    values ('OP_WA_0015',v_cli,(select id from ciudades where nombre='Pitalito' and departamento='Huila'),'B2C',v_wa,v_cola,false,'2026-07-14','Tiene un rack S15. Incluye ademas: barra olimpica negra 20kg, discos 15/25/35 lbs (~2 c/u). Vendedor Yohan S.') returning id into v_op;
  insert into op_items (op_id,producto_id,cantidad,precio_unit)
...
select o.numero, cl.nombre cliente, ci.nombre ciudad, o.fecha_entrega_pactada entrega,
  ...
  where o.numero like 'OP_WA_%' group by 1,2,3,4 order by o.numero;